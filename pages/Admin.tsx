  ...
  const saveSettings = async () => {
      ...
      try {
          ...
          if (activeTab === 'LIBRARY') {
              ...
              const updated = { ...settings, ftpSettings: ftpConfig };
              await db.updateSystemSettings(updated);
              setSettings(updated);
          } else {
      ...
      {activeTab === 'FTP' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-slate-900 p-6 rounded-xl border border-slate-800 space-y-4">
                  <h3 className="font-bold text-white flex items-center gap-2"><Network size={18}/> FTP Library Scan</h3>
                  <div className="p-4 bg-indigo-900/10 border border-indigo-500/20 rounded-lg text-sm text-indigo-200/80 leading-relaxed">
                      Use this if the Local Scan is blocked by permissions or <code>open_basedir</code>. 
                      <br/><br/>
                      The system will connect via FTP to list files. It will also use these credentials to stream videos if direct file access fails.
                      <br/>
                      <strong>Note:</strong> Set Host to <code>127.0.0.1</code> to connect to the NAS itself locally.
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">FTP Host</label>
                          <input type="text" value={ftpConfig.host} onChange={e=>setFtpConfig({...ftpConfig, host: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" placeholder="127.0.0.1" />
                      </div>
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Port</label>
                          <input type="number" value={ftpConfig.port} onChange={e=>setFtpConfig({...ftpConfig, port: parseInt(e.target.value)})} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" placeholder="21" />
                      </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                      <div>
                          ...
                          <input type="text" value={ftpConfig.user} onChange={e=>setFtpConfig({...ftpConfig, user: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" />
                      </div>
                      <div>
                          <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Password</label>
                          <input type="password" value={ftpConfig.pass} onChange={e=>setFtpConfig({...ftpConfig, pass: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" />
                      </div>
                  </div>

                  <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Root Path to Scan</label>
                      <input type="text" value={ftpConfig.rootPath} onChange={e=>setFtpConfig({...ftpConfig, rootPath: e.target.value})} className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-white" placeholder="/volume1/video" />
                  </div>

                  <button 
                    onClick={handleScanFtp}
                    disabled={scanningFtp}
                  ...
                  >
                      {scanningFtp ? <Loader2 className="animate-spin" size={20}/> : <Network size={20}/>}
                      {scanningFtp ? 'Scanning FTP...' : 'Save Config & Scan'}
                  </button>
              </div>
              ...
              <div className="bg-black p-4 rounded-xl border border-slate-800 font-mono text-xs text-slate-300 h-[500px] overflow-y-auto shadow-inner">
                  ...
                  {scanningFtp && <div className="animate-pulse">_</div>}
              </div>
          </div>
      )}
