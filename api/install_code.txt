...
// This file is copied by the build script from api/functions_schema.txt to dist/api/functions_schema.php
...
if ($action === 'check') {
    if (file_exists($configFile)) {
        ...
        try {
            ...
            // Simple check if key table exists
...
if ($action === 'install') {
    ...
    try {
        ...
        // Uses the function getAppSchema() and syncTable() from functions_schema.php
        $schema = getAppSchema();
        ...
        // Ensure Admin has ADMIN role if user already existed
        ...
        // --- GENERATE DEFAULT THUMBNAIL IF NOT EXISTS ---
        // This ensures the offline mode works without relying on external placeholders
        $defaultThumb = 'uploads/thumbnails/default.jpg';
        if (!file_exists($defaultThumb) && function_exists('imagecreatetruecolor')) {
            $im = imagecreatetruecolor(640, 360);
            $bg = imagecolorallocate($im, 15, 23, 42); // slate-950
            imagefill($im, 0, 0, $bg);
            $text_color = imagecolorallocate($im, 79, 70, 229); // indigo-600
            // Draw a simple pattern since we might not have fonts
            imagerectangle($im, 50, 50, 590, 310, $text_color);
            imageline($im, 50, 50, 590, 310, $text_color);
            imageline($im, 590, 50, 50, 310, $text_color);
            imagejpeg($im, $defaultThumb);
            imagedestroy($im);
        }
