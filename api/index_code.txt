...
set_time_limit(0); // Disable timeout for streams
...
// --- STREAMING HANDLER (TOP PRIORITY) ---
// Must happen before loading heavy libraries to prevent buffer pollution
if (isset($_GET['action']) && $_GET['action'] === 'stream') {
    ...
    if (file_exists($configFile)) {
        ...
        try {
            ...
            if ($row) {
                // Import only the streaming function to keep it lightweight
                require_once 'functions_utils.php';
                streamVideo($row['videoUrl'], $pdo);
                exit; // Stop execution immediately after streaming
            }
        } catch (Exception $e) {
...
require_once 'functions_payment.php'; // NEW
...
try {
    switch ($action) {
        ...
        case 'ftp_list': video_ftp_list($pdo, $input); break;
        case 'ftp_import': video_ftp_import($pdo, $input); break;
        case 'scan_ftp_recursive': video_ftp_scan_recursive($pdo, $input); break;
        case 'smart_organize': video_smart_organize($pdo); break;
        case 'rectify_titles': video_rectify_titles($pdo, $input); break;
        ...
        case 'admin_get_global_transactions': admin_get_global_transactions($pdo); break;
        case 'admin_handle_balance_request': admin_handle_balance_request($pdo, $input); break;
        case 'admin_handle_vip_request': admin_handle_vip_request($pdo, $input); break;
        ...
        case 'admin_update_request_status': admin_update_request_status($pdo, $input); break;
        ...
        case 'admin_get_real_stats': admin_get_real_stats($pdo); break; // ADDED THIS
