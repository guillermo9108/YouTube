...
// Encapsulado en función para evitar problemas de scope global
    ...
    return [
        ...
        'system_settings' => [
            'cols' => [
                ...
                'tropipayClientId' => 'VARCHAR(255) DEFAULT NULL',
                'tropipayClientSecret' => 'VARCHAR(255) DEFAULT NULL',
                'currencyConversion' => 'DECIMAL(10, 2) DEFAULT 1.00'
            ]
        ],
        'subscriptions' => [
            'cols' => [
...
function syncTable($pdo, $tableName, $def) {
    try {
        ...
        if ($result->rowCount() == 0) {
            ...
            $rawCols = $stmt->fetchAll(PDO::FETCH_COLUMN);
            $existingCols = array_map('strtolower', $rawCols);
