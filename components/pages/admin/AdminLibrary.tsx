...
const ScannerPlayer: React.FC<ScannerPlayerProps> = ({ video, onComplete }) => {
    ...
    // CRITICAL FIX: Ensure we use the stream proxy for local files to bypass browser security
    // and avoid "broken video" icon.
    let streamSrc = video.videoUrl;
    const isLocal = Boolean(video.isLocal) || (video as any).isLocal === 1 || (video as any).isLocal === "1";

    // Only transform if it's NOT already a stream URL
    ...
    useEffect(() => {
        ...
        const startPlay = async () => {
            try {
                ...
                // Force a small play attempt to ensure buffer starts
                await vid.play();
                setStatus('Procesando...');
            } catch (e) {
                console.warn("Autoplay blocked or load error", e);
                setStatus('Esperando click manual...');
                // Even if autoplay fails, metadata might load.
            }
        };
        
        // Small delay to ensure DOM is ready and previous src is cleared
    ...
    const handleTimeUpdate = (e: React.SyntheticEvent<HTMLVideoElement>) => {
        ...
        // Wait for sufficient data (skip black frames at 0s)
    ...
    const handleError = (e: any) => {
        ...
        console.error("Video Load Error for", streamSrc, e);
