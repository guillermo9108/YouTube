...
import { 
    ...
    Cpu, Globe, CreditCard, Settings2
} from 'lucide-react';
            ...
            <div className={`bg-slate-950 p-4 rounded-xl border transition-all ${level > 0 ? 'ml-6 border-l-indigo-500 border-slate-800' : 'border-slate-700'}`}>
                ...
                {showRules && (
                    <div className="mt-4 p-4 bg-slate-900 rounded-xl border border-indigo-500/20 space-y-4 animate-in slide-in-from-top-1">
                        <div>
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Keywords en Carpeta (NAS)</label>
                            ...
                            <input 
                                type="text" placeholder="Añadir keyword..."
...
export default function AdminConfig() {
    ...
    // Generador de IDs únicos reales basado en timestamp + random
    const generateUID = () => `ID_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`.toUpperCase();
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in pb-24 px-2">
            <div className="flex justify-between items-center px-2">
                <h2 className="text-xl font-black text-white uppercase tracking-tighter">Panel de Control</h2>
                <button 
                    onClick={handleSave} 
                    disabled={saving} 
            ...
            <ConfigSection title="Categorías y Precios" icon={FolderTree} isOpen={openSection === 'PRICES'} onToggle={() => setOpenSection(openSection === 'PRICES' ? '' : 'PRICES')}>
                <div className="space-y-4">
                    ...
                    >
                        <Plus size={18}/> Nueva Categoría Raíz
                    </button>
                </div>
            </ConfigSection>

            <ConfigSection title="IA & Metadatos (Gemini)" icon={Sparkles} isOpen={openSection === 'IA'} onToggle={() => setOpenSection(openSection === 'IA' ? '' : 'IA')}>
                <div className="space-y-4">
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                        ...
                        <input type="password" value={settings.geminiKey} onChange={e => setSettings({...settings, geminiKey: e.target.value})} className="w-full bg-transparent text-indigo-300 font-mono outline-none" placeholder="AIzaSy..." />
                    </div>
                </div>
            </ConfigSection>

            <ConfigSection title="Rutas de Sistema & Video" icon={Cpu} isOpen={openSection === 'SYSTEM'} onToggle={() => setOpenSection(openSection === 'SYSTEM' ? '' : 'SYSTEM')}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Ruta FFmpeg</label>
                        <input type="text" value={settings.ffmpegPath} onChange={e => setSettings({...settings, ffmpegPath: e.target.value})} className="w-full bg-transparent text-white font-mono outline-none" />
                    </div>
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Ruta yt-dlp</label>
                        <input type="text" value={settings.ytDlpPath} onChange={e => setSettings({...settings, ytDlpPath: e.target.value})} className="w-full bg-transparent text-white font-mono outline-none" />
                    </div>
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 col-span-2">
                        ...
                        <input type="text" value={settings.localLibraryPath} onChange={e => setSettings({...settings, localLibraryPath: e.target.value})} className="w-full bg-transparent text-indigo-300 font-mono outline-none" />
                    </div>
                </div>
            </ConfigSection>

            <ConfigSection title="Pasarela & Economía" icon={Landmark} isOpen={openSection === 'ECONOMY'} onToggle={() => setOpenSection(openSection === 'ECONOMY' ? '' : 'ECONOMY')}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Tropipay Client ID</label>
                        <input type="text" value={(settings as any).tropipayClientId || ''} onChange={e => setSettings({...settings, tropipayClientId: e.target.value} as any)} className="w-full bg-transparent text-white font-mono outline-none" />
                    </div>
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
