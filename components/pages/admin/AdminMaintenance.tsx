...
import { 
    ...
    Settings2, Info, AlertTriangle, Loader2, Play, Check, X, ShieldAlert, Zap, FileText, RefreshCw, BarChart3, HardDriveDownload
} from 'lucide-react';
    ...
    return (
        <div className="space-y-6 animate-in fade-in pb-20">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <SystemHealthCard icon={HardDrive} label="Escritura" status="OK" color="bg-purple-500" />
                <SystemHealthCard icon={Server} label="Background" status="Activo" color="bg-amber-500" />
            </div>
                ...
                <div className="lg:col-span-1 space-y-4">
                    <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h3 className="font-bold text-white">Janitor Engine V7</h3>
                        </div>
                    ...
                    {execution ? (
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden h-[600px] flex flex-col">
                            <div className="p-4 bg-slate-950 border-b border-slate-800 flex justify-between items-center">
                                <div className="flex items-center gap-2"><FileText size={18} className="text-indigo-400" /><h4 className="text-sm font-black text-white uppercase">Incidentes</h4></div>
                            </div>
                            <div className="flex-1 bg-black/40 overflow-y-auto p-4 font-mono text-[10px] space-y-1.5 custom-scrollbar">
                                {logs.map((log, i) => <div key={i} className="text-slate-400 border-b border-white/5 pb-1">{log}</div>)}
                            </div>
                        </div>
                    )}
                </div>
