...
export default function Home() {
  ...
  const breadcrumbPath = useMemo(() => {
      ...
      let current = hierarchy.find(h => h.name === activeCategory);
      while (current && current.parent) {
          path.unshift(current.parent);
          current = hierarchy.find(h => h.name === current.parent);
      }
      return path;
  ...
  const filteredList = useMemo(() => {
      return allVideos.filter(v => {
          ...
          const getAllDescendants = (cat: string): string[] => {
              ...
              let all = [...children];
              children.forEach(c => { all = [...all, ...getAllDescendants(c)]; });
              return all;
          };
          const descendants = [activeCategory, ...getAllDescendants(activeCategory)];
          const matchCat = descendants.includes(v.category);

          return matchSearch && matchCat;
      }).sort((a,b) => b.createdAt - a.createdAt);
  }, [allVideos, activeCategory, searchQuery, hierarchy]);
