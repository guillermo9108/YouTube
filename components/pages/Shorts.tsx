...
const ShortItem = ({ video, isActive, shouldLoad, preload, hasFullAccess }: ShortItemProps) => {
  ...
  if (!shouldLoad) return <div className="w-full h-full snap-start bg-black flex items-center justify-center"><Loader2 className="animate-spin text-slate-800" /></div>;
  ...
  return (
    <div className="relative w-full h-[100dvh] md:h-full snap-start snap-always shrink-0 flex items-center justify-center bg-black overflow-hidden">
          ...
          <>
           ...
           <div className="w-full h-full relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-20 p-6">
                 <div className="bg-black/40 backdrop-blur-xl p-8 rounded-[32px] border border-white/10 text-center w-full max-w-xs shadow-2xl">
                    <div className="text-4xl font-black text-amber-400 mb-6 tracking-tighter">{video.price} $</div>
                    <button onClick={() => db.purchaseVideo(user!.id, video.id).then(()=>setIsUnlocked(true))} className="w-full bg-white text-black font-black py-4 rounded-2xl shadow-xl uppercase text-xs tracking-widest active:scale-95 transition-all">Desbloquear</button>
                 </div>
              </div>
           </div>
        )}
      ...
      <div className="absolute right-2 bottom-20 z-30 flex flex-col items-center gap-5 pb-safe">
        ...
        <button onClick={(e) => { e.stopPropagation(); if(navigator.share) navigator.share({title: video.title, url: window.location.href}); }} className="w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-md bg-black/40 text-white"><Share2 size={26}/></button>
      </div>

      {/* Info Perfil Inferior Izquierda */}
      ...
      {showComments && (
        <div className="fixed inset-0 z-[100] flex items-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="w-full bg-slate-900 rounded-t-[32px] h-[70%] flex flex-col border-t border-slate-700 shadow-2xl animate-in slide-in-from-bottom" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={(e) => { e.preventDefault(); if(!newComment.trim()) return; db.addComment(user!.id, video.id, newComment).then(c => { setComments([c, ...comments]); setNewComment(''); }); }} className="p-4 bg-slate-950 border-t border-slate-800 flex gap-2 pb-safe">
                 <input type="text" value={newComment} onChange={e => setNewComment(e.target.value)} className="flex-1 bg-slate-900 border border-slate-700 rounded-full px-4 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500" placeholder="Escribe un comentario..." />
                 <button type="submit" disabled={!newComment.trim()} className="bg-indigo-600 text-white w-10 h-10 rounded-full flex items-center justify-center disabled:opacity-30 active:scale-90 transition-all"><Send size={18} /></button>
              </form>
           </div>
...
export default function Shorts() {
  ...
  useEffect(() => {
    db.getAllVideos().then((all: Video[]) => {
        // FILTRO CRÍTICO: Solo videos de < 3 min, que no sean PENDING/PROCESSING y cuya transcodificación no haya fallado
        const shorts = all.filter(v => 
            v.duration < 180 && 
            !['PENDING', 'PROCESSING', 'FAILED_METADATA', 'FAILED'].includes(v.category) &&
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => { if (entry.isIntersecting) setActiveIndex(Number((entry.target as HTMLElement).dataset.index)); });
  ...
  const hasFullAccess = useMemo(() => {
      ...
      return Boolean(user.role?.trim().toUpperCase() === 'ADMIN' || (user.vipExpiry && user.vipExpiry > (Date.now() / 1000)));
  }, [user]);
