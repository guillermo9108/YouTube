...
export default function GridProcessor() {
    ...
    const handleFailure = async (reason: string) => {
        ...
        console.error("Task failed:", activeTask.title, reason);
        try {
            // Tell backend this specific ID failed browser extraction
            await db.updateVideoMetadata(activeTask.id, 0, null, false);
        } catch(e) {}
        
        setTimeout(skipTask, 2000);
    };
    ...
    const handleTimeUpdate = (e: React.SyntheticEvent<HTMLVideoElement>) => {
        ...
        if (vid.currentTime > 1.5 && vid.videoWidth > 0) {
            ...
            try {
                ...
                if (ctx) {
                    ...
                    canvas.toBlob(async (blob) => {
                        ...
                        if (blob) {
                            ...
                            // Success call to backend (will trigger instant Step 3)
                ...
                handleFailure("Canvas error");
            }
        }
    };

    const handleError = () => {
        handleFailure("Video Load Error");
    };
    ...
    return (
        <div className="fixed bottom-20 right-4 md:bottom-6 md:right-6 z-50 animate-in slide-in-from-right fade-in duration-500">
            <div className={`bg-slate-900/95 backdrop-blur-md border ${status === 'ERROR' ? 'border-red-500/50' : 'border-slate-700'} rounded-lg shadow-2xl flex items-center p-2 gap-3 w-64 overflow-hidden relative`}>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center h-full">
                    <div className="flex items-center gap-1.5 mb-0.5">
                        {status === 'ERROR' ? (
                             <span className="text-[10px] font-black text-red-400 uppercase tracking-wide">ERROR DE ARCHIVO</span>
                        ) : (
                            <>
                                <Sparkles size={10} className="text-amber-400" />
                                <span className="text-[10px] font-black text-amber-400 uppercase tracking-wide">
                                    {status === 'DONE' ? '¡AGREGADO!' : 'PROCESANDO'}
                                </span>
                            </>
                        )}
                    </div>
